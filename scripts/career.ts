...
import type { PublicRecord } from './lib/model.ts';
import type { ScanOptions } from './lib/git.ts';
import { errorMessage, isObject } from './lib/files.ts';
import { parseLocalConfig, parseCandidate } from './lib/validation.ts';
...
import { buildSite, publishPublicDataset } from './lib/build.ts';
import { buildCandidatePreview } from './lib/preview.ts';
import { PATHS } from './lib/constants.ts';
import { calculateActivityDomainMix } from './lib/domains.ts';
import {
  countFiles,
  currentDate,
  pathExists,
...
function integerFlag(value: string | boolean | undefined, name: string) {
...
function printIssues(issues: string[], prefix = '  - ') {
...
Usage:
  node --experimental-strip-types scripts/career.ts <command> [options]
...
Commands:
  ...
  validate-private             Validate activities, workstreams, and legacy initiatives
  audit [--candidate <slug>]   Audit public, staged, or candidate content
  preview                      Build an isolated site from private candidates
...
async function commandDoctor() {
  ...
  const add = (level: string, message: string) => checks.push({ level, message });
  ...
  try {
    ...
    add('FAIL', `Unable to read public content: ${errorMessage(error)}`);
  }
  ...
  try {
    ...
    add('FAIL', `Unable to audit public content: ${errorMessage(error)}`);
  }
  ...
  if (!(await pathExists(PATHS.localConfig))) {
    ...
    try {
      ...
      add('FAIL', `Unable to inspect local configuration: ${errorMessage(error)}`);
    }
  }
...
async function commandScan(flags: Flags) {
...
async function latestFiles(directory: string, limit = 8) {
...
async function collectPendingOutcomes() {
  const files = [
    ...(await walkFiles(PATHS.activities)),
    ...(await walkFiles(PATHS.workstreams)),
    ...(await walkFiles(PATHS.initiatives))
  ];
  const pending = [];
  for (const file of files.filter((item) => item.endsWith('.json'))) {
    try {
      const value = await readJson(file);
      if (!isObject(value)) throw new Error('Invalid private record');
      const outcomes: unknown[] = [...(Array.isArray(value.outcomes) ? value.outcomes : []), ...(Array.isArray(value.potentialOutcomes) ? value.potentialOutcomes : [])];
      const count = outcomes.filter((outcome) => isObject(outcome) && outcome.status === 'needs-confirmation').length;
      if (count > 0) {
        pending.push({ file, title: typeof value.title === 'string' ? value.title : path.basename(file), count });
      }
    } catch {
...
async function commandReview() {
  const sections: Array<[string, string]> = [
    ...
    ['Drafts', PATHS.drafts],
    ['Legacy initiatives', PATHS.initiatives],
    ['Public candidates', PATHS.candidates],
    ['Outcome reviews', PATHS.outcomeReviews],
    ['Career projections', PATHS.projections]
  ];
  ...
  const activityFiles = (await walkFiles(PATHS.activities)).filter((file) => file.endsWith('.json'));
  const activities = await Promise.all(activityFiles.map((file) => readJson(file)));
  const domainMix = calculateActivityDomainMix(activities);
  console.log(`Recorded activity mix: ${domainMix.activityCount} activity record(s), with each activity split equally across its domains`);
  for (const item of domainMix.items) {
    console.log(`  ${item.domain}: ${item.percentage.toFixed(1)}%`);
  }
}
...
async function commandValidate() {
  ...
  console.log(`Public content valid: ${content.entries.length} entry record(s), ${content.projects.length} project record(s), ${content.resume ? '1 resume record' : 'no resume record'}.`);
}
...
async function commandValidatePrivate() {
  ...
  const activities = await countFiles(PATHS.activities, (file) => file.endsWith('.json'));
  const workstreams = await countFiles(PATHS.workstreams, (file) => file.endsWith('.json'));
  ...
  console.log(`Private records valid: ${activities} activity record(s), ${workstreams} workstream record(s), ${drafts} legacy draft(s), ${initiatives} legacy initiative(s).`);
}

async function findCandidate(slug: string): Promise<string> {
...
function validateCandidateRecord(value: unknown, file: string) {
...
async function commandAudit(flags: Flags) {
  if (typeof flags.candidate === 'string') {
    const file = await findCandidate(flags.candidate);
    const value = parseCandidate(await readJson(file), file);
...
function approvedDestination(value: PublicRecord) {
  ...
  throw new Error('Unsupported candidate record type.');
}

function publicSchemaPath(value: PublicRecord) {
  if (value.recordType === 'entry') return '../../../schemas/public-entry.schema.json';
  if (value.recordType === 'project') return '../../../schemas/public-project.schema.json';
  return '../../schemas/public-resume.schema.json';
}

function recordTypeDirectory(value: PublicRecord) {
  if (value.recordType === 'entry') return 'entries';
  if (value.recordType === 'project') return 'projects';
  return 'resume';
}

async function confirmApproval(value: PublicRecord) {
...
async function commandApprove(flags: Flags) {
  ...
  const approvedIssues = value.recordType === 'entry'
    ...
    : value.recordType === 'project'
      ? validateProject(approved, { mode: 'public', source: value.slug })
      : validateResume(approved, { mode: 'public', source: value.slug });
...
async function commandPublish() {
  ...
  console.log(`${result.data.stats.entries} entry record(s), ${result.data.stats.projects} project record(s), ${result.data.resume ? '1 resume record' : 'no resume record'}.`);
}

async function commandPreview() {
  const result = await buildCandidatePreview();
  console.log(`Private review site generated in ${toPosixPath(path.relative(PATHS.root, result.distDir))}/.`);
  console.log(`${result.candidateCount} candidate record(s), ${result.files.length} pages and assets.`);
  console.log('This preview is ignored by Git and is not a publication source.');
}
...
async function commandBuild() {
  ...
  console.log(`JavaScript gzip: ${(result.javascript.main / 1000).toFixed(1)} KB main / ${(result.javascript.architecture / 1000).toFixed(1)} KB lazy architecture`);
...
async function main() {
  ...
  if (!COMMANDS.has(command)) {
    throw new Error(`Unknown command "${command}". Run node --experimental-strip-types scripts/career.ts help.`);
  }
  ...
  switch (command) {
    ...
    case 'preview': await commandPreview(); break;
...
main().catch((error: unknown) => {
  console.error(`\n${error instanceof ValidationError ? errorMessage(error) : `Error: ${errorMessage(error)}`}`);
