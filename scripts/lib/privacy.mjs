...
import { LOCAL_ONLY_FILES, PATHS, PRIVATE_PATH_PREFIXES } from './constants.mjs';
import { pathExists, readJson, toPosixPath, walkFiles } from './files.mjs';
...
function compileCustomPatterns(patterns = []) {
  ...
  patterns.forEach((pattern, index) => {
    try {
      ...
      errors.push(`privacy.blockedPatterns[${index}]: ${error.message}`);
    }
  });
...
export function auditObject(value, options = {}) {
...
export async function auditPublicFiles(options = {}) {
...
export async function auditCandidateFile(file) {
...
export async function auditGeneratedDirectory(directory) {
  ...
  for (const file of files) {
    ...
    findings.push(...auditText(text, { source, includeTicketIds: true, includeScriptTags: false }));
  }
